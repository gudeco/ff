...
export function drawMariCat(ctx,p,images){
 ...
 const atlas=images[p.white?'mari-cat-white':'mari-cat-black'];
 const s=feedbackFrames(atlas)[mariCatFrame(p)],ground=p.y+20;
 ctx.save();ctx.fillStyle='#04090755';ctx.beginPath();ctx.ellipse(p.x,ground+2,35,5,0,0,Math.PI*2);ctx.fill();ctx.restore();
 feedbackPaint(ctx,s,p.x,ground,Math.sign(p.vx)||1,.26,275);
}
