...
import {Match,ROSTER,GROUND,ATTACKS,attackCost} from './combat.js';
...
let chosen='gudeco',stage='plaza',mode='cpu',match=null,paused=false,last=0,accumulator=0,clock=0,shake=0,flash=0,banner='',bannerTime=0,particles=[],floaters=[],soundOn=true,ready=false,songPreviewActive=false;
...
function syncAudioPause(){
 const backgroundPreview=songPreviewActive&&!match&&document.hidden;
 soundEngine.synthLookAhead=backgroundPreview?1.5:.22;
 soundEngine.setPaused(paused||(document.hidden&&!backgroundPreview)).catch(console.warn);
 if(backgroundPreview)soundEngine.schedule();
}
...
function start(){if(!ready)return;songPreviewActive=false;underpassCrowd=createUnderpassCrowd();floods=[];$('#song-next').hidden=true;$('#song-current').hidden=true;soundEngine.setTheme(stage);match=new Match(chosen,$('#opponent').value,mode,$('#difficulty').value);paused=false;accumulator=0;particles=[];floaters=[];keys.clear();taps.clear();$('#selection').hidden=true;$('#arena').hidden=false;$('#overlay').hidden=true;$('#stage-name').textContent=({plaza:'SQUARE',street:'STREET',hospital:'HOSPITAL',underpass:'UNDERPASS'})[stage];syncAudioPause();events();canvas.focus();}
...
function back(){songPreviewActive=false;match=null;paused=false;soundEngine.setTheme('title');$('#song-next').hidden=false;updateSongPreview();syncAudioPause();keys.clear();taps.clear();$('#arena').hidden=true;$('#selection').hidden=false;$('#overlay').hidden=true;}
...
$('#song-next').onclick=async()=>{if(match)return;songPreviewActive=true;soundEngine.setTheme(previewSongs[(previewSongs.indexOf(soundEngine.theme)+1)%previewSongs.length]);$('#song-current').hidden=false;updateSongPreview();if(!soundOn){soundOn=true;updateSoundButton();}await activateSound();syncAudioPause();};
...
function hud(){const g=ctx.createLinearGradient(0,0,0,190);g.addColorStop(0,'#07100ff2');g.addColorStop(1,'#07100f00');ctx.fillStyle=g;ctx.fillRect(0,0,1280,190);
 ...
 match.fighters.forEach((f,i)=>{const x=i===0?45:765;['Punch','Kick',...ROSTER[f.id].moves].forEach((move,n)=>{const px=x+n*96,enabled=(n<3||f.energy>=attackCost(f.id,n===3?'special1':'special2'))&&(n!==4||!match.hasActiveCats(i));ctx.strokeStyle=enabled?'#8d9172':'#414b3e';ctx.strokeRect(px,746,20,20);text((i===0?['J','U','I','K','L']:['1','2','3','4','5'])[n],px+10,760,10,enabled?'#e8d5a7':'#697163','center');text(move,px,782,9,enabled?'#d4cfb1':'#697163');});});
}
